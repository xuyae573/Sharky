...
using Sharky.Core;
using Sharky.Core.MultiTenancy;
using Sharky.Core.Reflection;
...
namespace Sharky.Domain.Entities
{
    ...
    public static class EntityHelper
    {
        ...
        public static void CheckEntity([NotNull] Type type)
        {
            ...
            if (!IsEntity(type))
            {
                throw new SharkyException($"Given {nameof(type)} is not an entity: {type.AssemblyQualifiedName}. It must implement {typeof(IEntity).AssemblyQualifiedName}.");
            }
        }
